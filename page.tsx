...
import HabilitaSection from "../habilita-section"
...
export default function MembersArea() {
  ...
  useEffect(() => {
    const calculateViews = () => {
      const now = new Date()
      const startOfDay = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 1, 0) // 00:01
      const endOfDay = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 0) // 23:59
    ...
    calculateViews()
    const interval = setInterval(calculateViews, 60000) // Atualiza a cada minuto
          ...
          <div className="flex items-center justify-center sm:justify-start">
            <div className="flex items-center space-x-3 sm:space-x-4">
              <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-gradient-to-r from-[#fd7ab1] to-pink-400 p-1 shadow-lg pulse-glow">
                <img
                  src="/chocolicias-logo-circular.png"
                  alt="Chocolícias da Fabi"
                  className="w-full h-full rounded-full object-cover bg-white"
                />
              </div>
              <div className="text-center sm:text-left">
                <h1 className="text-lg sm:text-2xl font-bold text-[#2b2b2b]">Chocolícias da Fabi</h1>
                <p className="text-xs sm:text-sm text-gray-600">Painel VIP 2025</p>
              </div>
            </div>
          </div>
        </div>
                  ...
                  <div key={content.id} className="group flex-shrink-0 w-56 sm:w-64">
                    <div className="glass-card rounded-xl p-3 shadow-lg border border-pink-200/50 hover:shadow-xl hover:scale-105 transition-all duration-300 hover:glass-card-strong">
                      ...
                      <div className="aspect-[3/4] rounded-lg overflow-hidden mb-3 shadow-md">
                        <img
                          src={content.cover || "/placeholder.svg"}
                          alt={content.title}
        ...
        <div className="container mx-auto px-4 sm:px-6 text-center">
          <div className="glass-card-strong rounded-3xl p-6 sm:p-8 shadow-xl border border-pink-200/50 max-w-2xl mx-auto">
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl overflow-hidden mx-auto mb-4 sm:mb-6 shadow-lg pulse-glow">
              <img src="/whatsapp-logo.png" alt="WhatsApp" className="w-full h-full object-cover" />
            </div>
            <h3 className="text-xl sm:text-2xl font-bold text-[#2b2b2b] mb-3 sm:mb-4">
              Acesse o Grupo VIP no WhatsApp
            </h3>
            <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">
              🔄 <strong>Página em Atualização:</strong> Estamos reorganizando e adicionando novamente todos os e-books
              e arquivos.
              <br />
              <br />
      ...
      <footer className="relative z-10 bg-[#2b2b2b]/95 backdrop-blur-md text-white py-6 sm:py-8">
        <div className="container mx-auto px-4 sm:px-6 text-center">
          <div className="flex items-center justify-center space-x-4 mb-4">
            <img
              src="/chocolicias-logo-circular.png" // Logo circular no rodapé
              alt="Chocolícias da Fabi"
          ...
          <p className="text-gray-300 text-xs sm:text-sm">
            © 2025 Chocolícias da Fabi. Transformando sonhos em realidade através da confeitaria.
          </p>
        </div>
      </footer>

      {/* Enhanced Views Counter Popup */}
      <div
